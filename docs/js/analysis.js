...
export function initAnalysis(containerEl) {
  ...
  if (allBtn) allBtn.onclick = () => { startAnalysisAll().catch(e => { console.error('startAnalysisAll error:', e); toast('全部分析失败: ' + e.message); }); };
  ...
  loadHistory();
  renderHistory();
  } catch (e) { console.error('initAnalysis error:', e); }
}
